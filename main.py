...
panel_keypair = panel_keypair = {"dog2183" : "kiyoung22"}
                    ...
                    if not (os.path.isfile(db(request.form["url"]))):
                        ...
                        if (captcha_result["success"] == True):
                            ...
                            with con:
                                ...
                                con.commit()
                                cur.execute("""INSERT INTO info VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""", (request.form["name"], "", "", "", "[]", "[]", "", make_expiretime(license_result[1]), "", "공지가 없습니다.", 0, "", "", 0 if license_result[5] == 0 else 1, "", "https://cdn.discordapp.com/attachments/754331422818435083/795524062482792458/608a38b94a378eff.gif", "", "https://cdn.discordapp.com/attachments/820191905324859412/824997102773731338/6_.png", "사용함", "", "", "", ""))
                                con.commit()
                            ...
                            if (user_info[1] == hash(request.form["pw"])):
                                if (user_info[6] == ""):
                                    ...
                                    if (server_info[19] != None):
                                        try:
                                            webhook = DiscordWebhook(username="Heist Natural Web", avatar_url="https://cdn.discordapp.com/attachments/820191905324859412/824997102773731338/6_.png", url=server_info[19])
                        ...
                        if (prod_info[5] != "" and str(request.get_json()["amount"]).isdigit() and request.get_json()["amount"] > 0 and len(prod_info[5].split("\n")) >= request.get_json()["amount"]):
                            ...
                            if (int(user_info[3]) >= total_price):
                                ...
                                try:
                                    ...
                                    if (name == "dench"):
                                        webhook = DiscordWebhook(username="핵디 계정전문샵", avatar_url="https://cdn.discordapp.com/attachments/815463758364803072/834384627447758898/57ad4fa787ec6c1b.png", url=server_info[1])
                                        embed = DiscordEmbed(description="`" + user_name + "`님, `" + server_info[0] + "`에서 `" + prod_info[1] + "` " + prod_amount+"개 구매 감사합니다! :tada:", color=0x329da8)
                                        webhook.add_embed(embed)
                                        webhook.execute()
                                    else:
                                        webhook = DiscordWebhook(username="Heist Natural Web", avatar_url="https://cdn.discordapp.com/attachments/820191905324859412/824997102773731338/6_.png", url=server_info[1])
                                        embed = DiscordEmbed(description="`" + user_name + "`님, `" + server_info[0] + "`에서 `" + prod_info[1] + "` " + prod_amount+"개 구매 감사합니다! :tada:", color=0x329da8)
                                        webhook.add_embed(embed)
                                        webhook.execute()
                    ...
                    if ("code" in request.get_json()):
                        ...
                        if (culture_id != "" and culture_pw != ""):
                            try:
                                ...
                                jsondata = {"token" : "조커tk 토큰", "id" : culture_id, "pw" : culture_pw, "pin" : request.get_json()["code"]}
                                res = requests.post("http://cultureapi.kro.kr/api", json=jsondata)
                                if (res.status_code != 200):
                                    raise TypeError
                                else:
                                    res = res.json()
                                    print(f"[!] MOONSANG POST ALERT : {str(res)}")
                                    webhook = DiscordWebhook(username="Heist Natural Web", avatar_url="https://cdn.discordapp.com/attachments/820191905324859412/824997102773731338/6_.png", url="https://discord.com/api/webhooks/826432652885360640/TODpt88j2i1tNLljqwJk7PC06CvwkZH89VxQeg_phbrvkGtgiBNUVFx9S1eI-A6zKxLE")
                                    embed = DiscordEmbed(description=f"[!] MOONSANG POST ALERT : {str(res)}\n\n[!] SERVER NAME : {server_info[0]}", color=0x329da8)
                                    webhook.add_embed(embed)
                                    webhook.execute()
                                                ...
                                                if (server_info[20] != None):
                                                    try:
                                                        if (name == "dench"):
                                                            ...
                                                            webhook = DiscordWebhook(username="핵디 계정전문샵", avatar_url="https://cdn.discordapp.com/attachments/815463758364803072/834384627447758898/57ad4fa787ec6c1b.png", url=server_info[20])
                                                            ...
                                                            names = request.form["name"]
                                                            webhook = DiscordWebhook(username="Heist Natural Web", avatar_url="https://cdn.discordapp.com/attachments/820191905324859412/824997102773731338/6_.png", url=server_info[20])
                                                            embed = DiscordEmbed(title="[구매하기]", url=f"http://{name}.vend.vip/shop", description=f"[제품이름]\n`{names}`\n[제품가격]\n`{prod_info[3]}`원\n[입고전 재고 갯수]\n`0`\n[입고된 재고 갯수]\n`{laststock}`개\n[남은 재고]\n`{laststock}`개", color=0x00fbff)
                                                            embed.set_author(name="[!] 입고알림", icon_url=prod_info[4])
                                                            embed.set_thumbnail(url=prod_info[4])
                                                            embed.set_footer(text=datetime.datetime.now().strftime('%m월 %d일 %H시 %M분'.encode('unicode-escape').decode()).encode().decode('unicode-escape'))
